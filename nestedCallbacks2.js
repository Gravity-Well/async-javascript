...
request(`https://maciejtreder.github.io/asynchronous-javascript/directors`, {json: true}, processDirectors);

function processDirectors(err, res, directors){
    let tarantinoId = directors.find(director => director.name === "Quentin Tarantino").id;
    request(`https://maciejtreder.github.io/asynchronous-javascript/directors/${tarantinoId}/movies`, {json: true}, processMovies);
}
function processMovies(err, res, movies){
...
function processreviews(err, res, reviews){
